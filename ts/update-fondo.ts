function updateFondo(game: GameScene) {
  ...
  let x2 = game.object<Phaser.GameObjects.Image>("fondo.2.a").setX(calcularPos(camera.x, 0.04, game.scale.width)).x;
  game.object<Phaser.GameObjects.Image>("fondo.2.b").setX(x2 + game.scale.width);
  let x3 = game.object<Phaser.GameObjects.Image>("fondo.3.a").setX(calcularPos(camera.x, 0.08, game.scale.width)).x;
  game.object<Phaser.GameObjects.Image>("fondo.3.b").setX(x3 + game.scale.width);
  let x4 = game.object<Phaser.GameObjects.Image>("fondo.4.a").setX(calcularPos(camera.x, 0.16, game.scale.width)).x;
